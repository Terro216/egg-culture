...
- **Стилизация:** CSS Modules. Библиотека компонентов — Shadcn UI (по мере необходимости для инпутов/модалок, но с сильной кастомизацией под дизайн-код).
...
2. **Базовый Layout & Дизайн-система:**
   - Настроить цвета (background `#F0EAD6`, акценты) в глобальных CSS переменных.
   - Подключить тонкий Serif шрифт (например, Playfair Display, Cormorant Garamond или аналогичный).
   - Создать общую обертку Layout, хедер с минималистичной навигацией и футер.
3. **Реализация Главной страницы:** Сделать 3D-анимацию яйца / параллакс, разместить ключевые хуки из манифеста.
4. **Страница Манифеста:** Сверстать манифест с акцентом на типографику и ритм чтения.
5. **Интерактивные виджеты (React):** Разработать Колесо дескрипторов и Калькуляторы температуры/времени.
6. **Роутинг и контент:** Создать страницы для Мерча, Карты Яй Ци, Блога и Афиши и заглушки картинок.
7. **Секретная зона:** Реализовать мини-квиз, сохраняющий состояние в `localStorage` или cookie, открывающий доступ к "The Dark Side of the Shell".
8. **Докеризация:** Написать `Dockerfile`, `docker-compose.yml`, подготовить скрипты деплоя в общую инфраструктуру.
