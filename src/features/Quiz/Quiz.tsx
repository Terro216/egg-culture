...
export const Quiz: React.FC<QuizProps> = ({ lang }) => {
  ...
  const [result, setResult] = useState<{ score: number; mistakes: number[] } | null>(null);
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (isAdept === 'true') {
        window.location.href = `/${lang}/dark-side`;
      }
    }
  }, [lang]);
  ...
  if (result) {
    return (
      <div className="quiz-result">
        ...
        <p>Вернитесь к супермаркетной полке. Ваше восприятие еще слишком фрагментарно для Истины.</p>
        <div className="result-stats">
          <p>Правильно: {result.score} / {questions.length}</p>
          <p>Ваше звание: <strong>{getRank(result.mistakes.length)}</strong></p>
        </div>
        ...
        <button className="quiz-btn" onClick={handleRetry}>Начать заново</button>
      </div>
    );
  }
  ...
  return (
    <div className="quiz-container">
      <p className="quiz-instruction">Инструкция: Выберите один верный ответ. Для прохождения необходимо ответить правильно на все вопросы.</p>
